...
export const updateFollow = async (follower, followee, action) => {
  try {
    switch (action) {
      ...
      case 'unfollow': {
        await axios.put(
          `${rootURL}/user/${follower.realUserId}/unfollow/${followee._id}`,
        );
...
export const updateLikes = async (currentUser, postId, action) => {
  try {
    ...
    switch (action) {
      case 'like': {
        console.log('currentuser id', currentUser.realUserId);
        console.log('like list:', post.likes);
        post.likes.push(currentUser.realUserId);
        console.log('like list:', post.likes);
        await axios.put(
      ...
      case 'unlike': {
        const index = post.likes.indexOf(currentUser.realUserId);
