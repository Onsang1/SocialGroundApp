...
export const updateFollowerSuggestions = async (current, usersData ) => {
  try {
    ...
    const suggestions = defaultData.filter((el) => {
      const followInCommon = [];
      if (el.following && el.following.length > 0 && current.following && current.following > 0){
    ...
    const response = await axios.put(`${rootURL}/suggestions/${current._id}`, {suggested:suggest});
    // console.log(response.data.data);
    return response.data.data;
  } catch (err) {
    console.error(err);
