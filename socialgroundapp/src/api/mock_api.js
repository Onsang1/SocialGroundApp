...
export const createUser = async (userObject) => {
  try {
    const response = await axios.post(
      `${rootURL}/users`,
      `id=${userObject.id}&username=${userObject.username}&name=${userObject.name}&email=${userObject.email}&password=${userObject.password}`,
    );
    return response.data;
...
export const createPost = async (file) => {
  try {
    ...
    reader.onloadend = async function onloadend() {
      const response = await axios.post(`${rootURL}/posts`, { id: Math.floor(Math.random() * 1000), content: reader.result });
      return response.data;
    };
