...
describe('POST /posts/ endpoint tests', () => {
    ...
    let testPostId;
    const testPost = {
      content: "This is a jest testing post!",
    ...
    beforeAll(async () => {
      ...
      response = await request(webapp).post('/posts/')
        .send('content=This is a jest testing post!&userId=6389982a373e83308395e820');
   ...
   */
    test('the new post is in the db', async () => {
      const insertedPost = await db.collection('posts').findOne({ content: 'This is a jest testing post!' });
      expect(insertedPost.userId).toEqual('6376d6d927d24d3dd3e4d88a');
    });
    ...
    test('Get all posts endpoint status code and data', async () => {
      ...
      const post = JSON.parse(resp.text).data;
      // testPost is in the response
      expect(post).toEqual(expect.arrayContaining([{ _id: testPostId, ...testPost }]));
    });
