...
//+++++++++++++++++
// 121722
webapp.get('/user/email/:email', async (req, res) => {
...
webapp.get('/user/:id', async (req, res) => {
  ...
  try {
    ...
    console.log(results);
    console.log('done server get user');
    // eslint-disable-next-line no-bitwise
    if (results === null) {
      res.status(404).json({ error: 'unknown user' });
      return;
...
webapp.post('/login', (req, res) => {
  ...
  try {
    const jwtoken = jwt.sign({ email: req.body.email }, secret, { expiresIn: '1h' });
...
webapp.post('/hold/:id', async (req, res) => {
  ...
  // sign the token and send it to the frontend
  try {
    const result = await dbLib.setAccountOnHold(req.body.id);
