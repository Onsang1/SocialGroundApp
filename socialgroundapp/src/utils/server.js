...
webapp.post('/login', (req, res) => {
  ...
  try {
    const jwtoken = jwt.sign({ username: req.body.email }, secret, { expiresIn: '30d' });
...
webapp.post('/currentUser/', async (req, res) => {
  ...
  try {
    const currUser = {
      ...
      suggested: ['6389982a373e83308395e820', '638a6556f3d7adc2733aec7d', '638aa82c698c2c18f20de209'],
    };
...
webapp.post('/posts/', async (req, res) => {
  ...
  // parse the body of the request
  if (!req.body.content || !req.body.userId
    || !req.body.author || !req.body.caption || !req.body.comments) {
    res.status(404).json({ message: 'missing information' });
    console.log('ADD A POST CREATES AN ERROR NOWWWWWWWWWWWWW');
    return;
  }
  try {
    // create the new comment
    console.log('----------------LINE 503 REACHED---------------------');
    const newPost = {
      content: req.body.content,
      userId: req.body.userId,
