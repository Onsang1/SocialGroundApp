...
//   } catch (err) {
//     res.status(409).json({ message: 'Fail to add current user' });
//   }
// });
 ...
 */
 ...
 //curl -X POST http://localhost:8080/posts/6376e52e15dd8a7a841cd930/comments/ -d "content=HAHAHAHAH&userId=638aa873698c2c18f20de20a"
//   webapp.post('/posts/:postId/comments/', async (req, res) => {
//   console.log('ATTACH comment to a post');
...
// implement the POST posts endpoint
  ...
  // parse the body of the request
  if (!req.body.content || !req.body.userId || !req.body.author || !req.body.caption ) {
    res.status(404).json({ message: 'missing information' });
    return;
  }
  try {
