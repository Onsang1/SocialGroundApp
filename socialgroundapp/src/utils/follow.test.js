...
describe('POST /users/:id/follow endpoint tests', () => {
  ...
  test('update follow', async () => {
    ...
    const res = await request(webapp).put(`/user/${currentUserId}/follow/${followeeId}`);
    expect(res.status).toEqual(200);
  });

  test('Get following list of current user', async () => {
    const followeeId = '6376d6d927d24d3dd3e4d88a';
    const resp = await request(webapp).get('/currentUser/following');
  ...
  test('update unfollow', async () => {
    const currentUserId = '638beb5a6d818a6c63d82344';
    const followeeId = '6376d6d927d24d3dd3e4d88a';
    const resp = await request(webapp).put(`/user/${currentUserId}/unfollow/${followeeId}`);
    expect(resp.status).toEqual(200);
    const resp2 = await request(webapp).get('/currentUser/following');
  ...
  test('update fails 409', async () => {
    ...
    const resp = await request(webapp).put(`/user/${currentUserId}/follow/${followeeId}`);
    expect(resp.status).toEqual(200);
  });
});
