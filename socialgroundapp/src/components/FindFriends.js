...
import { getUser, getUsers, updateFollowerSuggestions } from '../api/mock_api';
...
function Find() {
  ...
  useEffect(() => {
    ...
    const setFollowerSuggestions = async () => {
      console.log('updating following suggestions in find');
      console.log(`current: ${currentUser}`);
      await updateFollowerSuggestions(currentUser, users);
    };
    fetchUserData()
      .catch(console.error);
    fetchCurrData()
  ...
  const { listType, current } = location.state;
