...
function List({
  ...
  if (listType === 'suggestion') {
    ...
    const suggested = users.data.filter((el) => {
      // eslint-disable-next-line react/prop-types
      if (current.data.suggested.includes(el._id)) {
        console.log(el);
        return el;
      }
    ...
    const filteredData = users.data.filter((el) => {
    ...
    const followers = users.data.filter((el) => {
      if (current.data.followers?.includes(el._id)) {
        return el;
      }
      return null;
    });
    ...
    const following = users.data.filter((el) => {
      if (current.data.following?.includes(el._id)) {
        return el;
      }
      return null;
    });
