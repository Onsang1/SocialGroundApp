...
function List({
  ...
  if (listType === 'suggestion') {
    ...
    // console.log(users);
    const defaultData = users.filter( (el) => el._id != current._id && !current.following.includes(el._id));  // Exclude self
    
    const suggestions = defaultData.filter((el) => {
      if (el.following && el.following.length > 0 && current.following && current.following.length > 0){
