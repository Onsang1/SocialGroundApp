...
import { deletePosts, editCaption, getUser, addHiddenUserToPost } from '../api/mock_api';
...
function Post({
  ...
  async function handleHidePost(e){
    const user = await getUser(sessionStorage.getItem('user').slice(1, -1));
    console.log('user in post component', user._id)
    addHiddenUserToPost(e, user._id);
    console.log('add hidden user to post');
  }
